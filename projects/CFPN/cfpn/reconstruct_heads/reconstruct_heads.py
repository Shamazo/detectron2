...
class ReconstructHeads(torch.nn.Module):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.loss_weights = cfg.MODEL.RECONSTRUCT_HEADS.LOSS_WEIGHTS
...
class MLSPHead(ReconstructHeads):
    """
    Multi level reconstruction head using the features from multiple levels of
    the FPN backbone and two subpixel convolutions
    """
    ...
    def forward(
        ...
        for features, reconstruct_conv, output_conv, stage, weight_loss in zip(
            ...
            losses["img_{}_MSE".format(stage)] = weight_loss * self.loss(reconstructed_images * ds_mask, ds_images)
            results["img_{}".format(stage)] = reconstructed_images

        return results, losses


def ReconstructConvBlock(in_channels=512, out_channels=512, kernel=3, stride=1, padding=1) -> nn.Sequential:
    return nn.Sequential(
...
class SubPixelResidualReconstruct(nn.Module):
    def __init__(self, in_channels=512):
        super(SubPixelResidualReconstruct, self).__init__()
        self.cb1 = ReconstructConvBlock(in_channels=in_channels, out_channels=in_channels)
        self.cb2 = ReconstructConvBlock(in_channels=in_channels, out_channels=in_channels)
        self.sp1 = Subpixel(in_channels=in_channels, out_channels=256, scale=2)
        self.sp2 = Subpixel(in_channels=64, out_channels=12, scale=2)
...
@RECONSTRUCT_HEADS_REGISTRY.register()
class MLResidualHead(ReconstructHeads):
    """
    Multi level reconstruction head using the features from multiple levels of
    the FPN backbone and two subpixel convolutions
    """
    def __init__(self, cfg, input_shape, fuse_type='sum'):
        ...
        for idx, key in enumerate(self.in_features):
            ...
            reconstruct_conv = SubPixelResidualReconstruct(self.in_channels)
            ...
            stage = int(key[1])
            stages.append(stage)
            self.add_module("MLResSP_recon{}".format(stage), reconstruct_conv)
            self.add_module("MLResSP_output{}".format(stage), output_conv)
        # Place convs into top-down order (from low to high resolution)
        # to make the top-down computation in forward clearer.
        self.stages = stages[::-1]
        self.reconstruct_convs = reconstruct_convs[::-1]
